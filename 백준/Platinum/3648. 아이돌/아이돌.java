...
public class Main {
    ...
    public static void main(String[] args) {
		...
		while (sc.hasNext()) {
			...
			graph.get(toIndex(-1)).add(toIndex(1)); // ¬x₁ → x₁(상근이 반드시 진출)
			revGraph.get(toIndex(1)).add(toIndex(-1));
	        ...
	        // 4단계: 변수 값 할당 (i=1…varCount)
	        int[] answer = new int[varCount];
	        for (int i = 1; i <= varCount; i++) {
	        // x_i 정점은 2*i, not(x_i)는 2*i+1
	        answer[i - 1] = (sccId[2 * i] > sccId[2 * i + 1]) ? 1 : 0;
	        }
	        

	        // 출력
	        int sanggeunTrue = sccId[toIndex(1)];
	        int sanggeunFalse = sccId[toIndex(-1)];
	        if (can == 1 && sanggeunTrue > sanggeunFalse) {
